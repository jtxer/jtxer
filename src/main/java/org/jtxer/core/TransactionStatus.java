...
public class TransactionStatus {
	...
	public void setRollbackOnly(Throwable th) {
		if (isNoTransaction() == false) {
			if ((status != Status.STATUS_MARKED_ROLLBACK) && (status != Status.STATUS_ACTIVE)) {
				throw new JtxerException("No active TX that can be marked as rollback only");
			}
		}
		rollbackCause = th;
	...
	protected void checkTimeout() {
		...
		if (this.deadline - System.currentTimeMillis() < 0) {
			setRollbackOnly();
			throw new JtxerException("TX timed out, marked as rollback only");
		}
	}
	...
	protected void commitOrRollback(boolean doCommit) {
		...
		if (isNoTransaction() == false) {
			if (isRollbackOnly()) {
				...
				if (isCompleted()) {
					throw new JtxerException("TX is already completed, commit or rollback should be called once per TX");
				}
				throw new JtxerException("No active TX to " + (doCommit ? "commit" : "rollback"));
			}
		}
		if (doCommit == true) {
			commitAllResources();
	...
	protected void commitAllResources() {
		...
		if (lastException != null) {
			...
			throw new JtxerException("Commit failed: one or more TX resources couldn't commit a TX", lastException);
		}
	...
	protected void rollbackAllResources(boolean wasForced) {
		...
		if (lastException != null) {
			...
			throw new JtxerException("Rollback failed: one or more TX resources couldn't rollback a TX", lastException);
		}
		if (wasForced) {
			throw new JtxerException("TX rolled back because it has been marked as rollback-only", rollbackCause);
		}
	}
	...
	public <E> E requestResource(Class<E> resourceType) {
		if (isCompleted()) {
			throw new JtxerException("TX is already completed, resource are not available after commit or rollback");
		}
		if (isRollbackOnly()) {
			throw new JtxerException("TX is marked as rollback only, resource are not available", rollbackCause);
		}
		if (!isNoTransaction() && !isActive()) {
			throw new JtxerException("Resources are not available since TX is not active");
		}
		checkTimeout();
		...
		if (resource == null) {
			...
			if ((maxResources != -1) && (resources.size() >= maxResources)) {
				throw new JtxerException("TX already has attached max. number of resources");
			}
	...
	protected <E> E lookupResource(Class<E> resourceType) {
		for (ResourceAccess<?> resource : resources) {
			if (ClassHelper.isSubclass(resourceType, resource.getResource().getClass())) {
